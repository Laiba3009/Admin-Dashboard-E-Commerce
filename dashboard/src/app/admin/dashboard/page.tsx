...
export default function AdminDashboard() {
  ...
  // Logout function
  const handleLogout = () => {
    // Clear session data (e.g., clear from localStorage, cookies, or context)
    localStorage.removeItem("adminLoggedIn"); // Adjust this if using a different session storage method.

    // Redirect to login page
    router.push("/login");  // Redirect to your login page route
  };
  ...
  return (
    <ProtectedRoute>
      <div className="flex flex-col h-screen bg-gray-100">
        {/* Navbar */}
        <nav className="bg-purple-900 text-white p-4 shadow-lg flex justify-between">
          <h2 className="text-2xl font-bold">Admin Dashboard</h2>
          <div className="flex space-x-4">
            {["All", "pending", "dispatch", "success"].map((status) => (
              <button
                key={status}
                className={`px-4 py-2 rounded-lg transition-all ${filter === status ? "bg-white text-purple-900 font-bold" : "text-white"}`}
            ...
            <button
              ...
              className="px-4 py-2  text-white rounded-lg hover:bg-purple-900"
            >
              Logout
            </button>
          </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredOrders.map((order) => (
                  <React.Fragment key={order._id}>
                    <tr
                      className="cursor-pointer hover:bg-purple-300 transition-all"
